...
// 在树中查找数据
bool BPlusTree::Search(KEY_TYPE data, bool print)
{
	int i = 0;
    ...
    {        
        ...
        if(print)
            cout << setw(4) << pNode->GetElement(1) << " -->";
...
bool BPlusTree::Delete(KEY_TYPE data)
{
    ...
    if (pOldNode->GetCount() >= ORDER_V)
    {
        ...
        while(pNode){
            ...
            {
            // 如果删除的是树中的键值，需要更改该键
                if (pNode->GetElement(i) == data){
                    pNode->SetElement(i, pOldNode->GetElement(1)); // 更改为叶子结点新的第一个元素
                    return true;
                }
            }
            pNode = pNode->GetPointer(i);
...
void BPlusTree::PrintNode(CNode* pNode)
{
    ...
    {
        cout << setw(4) << pNode->GetElement(i);
        //if (i >= MAXNUM_KEY)
        //    cout<<" | ";
    }
}
...
bool BPlusTree::DeleteInternalNode(CInternalNode* pNode, KEY_TYPE key)
{
    ...
    if (pNode->GetCount() >= ORDER_V)
    {
        ...
        while(p){
            int i;
            for (i = 1; (i <= p->GetCount()) && (key >= p->GetElement(i)); i++)
            {
            // 如果删除的是树中的键值，需要更改该键
                if (p->GetElement(i) == key){
                    p->SetElement(i, pNode->GetElement(1)); // 更改为叶子结点新的第一个元素
                    return true;
                }
            }
            p = p->GetPointer(i);
